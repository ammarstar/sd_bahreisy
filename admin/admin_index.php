...
<div class="container">
  ...
  <!-- contents -->
  <form class="admin_contents" method="post" enctype="multipart/form-data">
    <div class="form-group">
      <label for="judul">Judul</label>
      <input type="text" placeholder="Judul Berita" class="form-control" name="judul" value="">
      <label for="isi">Isi Berita</label>
      <textarea name="isi" class="ckeditor form-control" rows="10" style="resize:none;"></textarea>
      <input type="text" placeholder="nama gambar" class="mt-3" name="nama_gambar" value="">
      <input type="file" name="gambar" value="" required>
    </div>
    <button type="submit" class="btn btn-info mb-5" name="simpan">Simpan</button>
  </form>
  <?php
  ...
  if (isset($_POST['simpan'])) {
    ...
    $query=mysqli_query($link, "insert into tbl_utama values(null,
                                                              '".$_POST['judul']."',
                                                              '".$_POST['isi']."',
                                                              '".$_POST['nama_gambar']."',
                                                              '$name')");
