...
<div class="content ">
  <div class="row d-flex justify-content-center">
    <div class="col-md-5 border border-dark m-3">
      <h5>pendaftaran daring</h5>
      <form class="form-group" method="post">
        <label for="nama">nama siswa</label>
        <input type="text" class="form-control" name="nama_siswa" value="">
        <label for="">Tgl/ Bln/ Thn</label>
        <input type="date" class="form-control" name="tanggal" value="">
        <label for="">Alamat Siswa</label>
        <input type="text" class="form-control" name="alamat_siswa" value="">
        <label for="">Nama Wali</label>
        <input type="text" class="form-control" name="nama_wali" value="">
        <label for="">Alamat Wali</label>
        <input type="text" class="form-control" name="alamat_wali" value="">
        <label for="">No.Telp Wali</label>
        <input type="text" class="form-control" name="telp" value="">
        <button type="submit" class="btn btn-primary form-control mt-2 mb-2" name="simpan">Simpan</button>
      </form>
      <?php
      include 'koneksi/connect.php';
      if (isset($_POST['simpan'])) {
        $query=mysqli_query($link, "insert into tbl_pendaftaran values(null,
                                                                      '".$_POST['nama_siswa']."',
                                                                      '".$_POST['tanggal']."',
                                                                      '".$_POST['alamat_siswa']."',
                                                                      '".$_POST['nama_wali']."',
